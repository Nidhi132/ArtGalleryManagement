...
session_start();
 $aidErr = $eidErr = $fnameErr = $lnameErr = $placeErr = $phoneErr = $styleErr = $mailidErr = $pw1Err = "";
 $aid = $eid = $fname = $lname = $place = $phone = $style = $mailid = "";
 $n=0;
...
if (empty($_POST["EID"])) {
    ...
    if (!preg_match("/^[E][0-9a-zA-Z]*$/",$eid)) {
      $eidErr = "Enter proper eid(Starting with E---...)";
    }
    else {
        $n++;}
}
...
<form ACTION="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" METHOD="POST">
  <div class="container">
    ...
    <hr>
    <center>
    <label for="artistid"><b>Artist ID</b></label><span class="error">* <?php echo $aidErr;?></span><br>
    <input type="text" placeholder="Enter Artist ID" name="AID" required value="<?= $aid; ?>"><br>
...
<p style="font-family: arial; "><a href="artist1.html"><font style="color:gold">GO BACK</font></a></p>
 ...
 <?php
    // if(isset($_POST['AID']) && isset($_POST['FNAME']) && isset($_POST['LNAME'])):
    ...
    $sql3 = "INSERT INTO ARTIST(aid, eid, fname, lname, place, style, mailid1, pw1) VALUES('".$aid."', '".$eid."', '".$fname."', '".$lname."', '".$place."',  '".$style."', '".$mailid."', '".$pw1."')";

    $result = $link->query($sql3);

     $sql3 = "INSERT INTO acontacts(aid, phone1) VALUES('".$aid."', '".$phone."')";
