...
<body style="background-color: lavenderblush">
	...
	<form action="edelete.php" method="POST">
		<div>Enter Exhibition ID:<input type="text" name="EID" placeholder="EID"><br></div>
		<br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    if($a!="")
        {
            $sql1 = "SELECT * from Exhibition where eid='$a' AND GID='$id'";
            $result = mysqli_query($con,$sql1);

            if(mysqli_num_rows($result)>0)
            {
            $sql3="DELETE from Exhibition where eid='$a' AND GID='$id'";   // student is the table name
            mysqli_query($con,$sql3);
            echo "<b>Record with EID = $a is deleted successfully.<b>";
            }
           else
        {
            echo "<b>!!!Error in Deleting Record!!!<br> Record '$a' didnot have gid '$id' so was not found in database.<b>" ;
        }
        }else{
