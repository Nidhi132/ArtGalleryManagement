...
<body style="background-color: lavender">
 ...
 <?php
$conn = mysqli_connect("127.0.0.1:3307", "root", "", "art_gallery");

  if ($conn->connect_error) {
   die("Connection failed: " . $conn->connect_error);
  } 

  $sql = "select * from artwork a, bid b where a.artid=b.artid and a.aid='$id'";
  ...
  if ($result->num_rows > 0)
   ...
   while($row = $result->fetch_assoc())
    ...
    echo "<tr><td>" . $row["ARTID"]. "</td><td>" . $row["TITLE"]. "</td><td>" . $row["YEAR"]. "</td><td>" . $row["TYPE"]. "</td><td>" . $row["INITIALPRICE"]. "</td><td>" . $row["STARTTIME"]. "</td><td>" . $row["ENDTIME"]. "</td><td>"
    . $row["AID"]. "</td><td><a href= ".$folder." >".$row["IMAGE"]."</td></tr>";
    }
    echo "</table>";
