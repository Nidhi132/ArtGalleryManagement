...
echo "<select name= 'AID' value='<?= $aid; ?>'>";
echo '<option value="">'.'--- Please Select Artist ID ---'.'</option>';

$sql="select AID from artist";
...
echo '</select>';
		?>
            ...
            <!-- <input type="text" name="AID" placeholder="AID" value="<?= $aid; ?>"> -->
			<br></div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    // if($n==1){
	$aid=$_POST['AID'];
	echo "<b><br>Entered Artist ID is $aid <br></b>";
	
	$sql="SELECT * from artist a, acontacts ac where a.aid=ac.aid and a.aid='$aid'";

				$result = $con->query($sql);

			if ($result->num_rows > 0) {
                echo "<b><br>Search Successful<br><br></b>";
		    echo "<br><br><br><br><table><tr><th>Artist ID</th><th>EID</th><th>FName</th><th>LName</th><th>Place</th><th>Style</th><th>Phone</th><th>Mail ID</th></tr>";
		   		    while($row = $result->fetch_assoc()) {
